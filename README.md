...
VyManager uses a **single unified `.env` file** at the project root for all configuration.
...
# Authentication (ALREADY SET if you ran the command above)
...
# Internal API URL (Docker only - leave as-is for Docker deployments)
...
> 📝 **Note**: See `.env.example` for detailed documentation on each setting, including different deployment scenarios (Docker, non-Docker, development).
