...
Each user can enable an authenticator app from Security in Site Manager (or Administration on an appliance). Sign-in then asks for a TOTP code, a backup code, or an email one-time code when SMTP is configured (`SMTP_HOST` and `SMTP_FROM`). SSO logins are not challenged. API tokens skip the second factor.
