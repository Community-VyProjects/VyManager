...
class FirewallIPv6Mapper(BaseFeatureMapper):
    ...
    def get_rule_set_hop_limit(self, chain: str, rule_number: int, hop_limit: str, is_custom: bool = False) -> List[str]:
        """Get command path for setting hop-limit value (IPv6 equivalent of TTL)."""
        if is_custom:
            return ["firewall", "ipv6", "name", chain, "rule", str(rule_number), "set", "hop-limit", hop_limit]
        return ["firewall", "ipv6", chain, "filter", "rule", str(rule_number), "set", "hop-limit", hop_limit]
    ...
    def get_rule_icmpv6_type_name(self, chain: str, rule_number: int, icmpv6_type: str, is_custom: bool = False) -> List[str]:
        """Get command path for setting ICMPv6 type name."""
        if is_custom:
            return ["firewall", "ipv6", "name", chain, "rule", str(rule_number), "icmpv6", "type", icmpv6_type]
        return ["firewall", "ipv6", chain, "filter", "rule", str(rule_number), "icmpv6", "type", icmpv6_type]
