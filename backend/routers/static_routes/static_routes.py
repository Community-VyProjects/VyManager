...
async def get_static_routes_capabilities():
    """
...
async def get_static_routes_config(refresh: bool = False):
    """
...
@router.delete("/route/{route_type}/{destination}")
async def delete_static_route(
    ...
    Args:
        ...
        table_id: Optional routing table ID
    """
    if CONFIGURED_DEVICE_NAME is None:
        raise HTTPException(status_code=503, detail="No device configured.")
    ...
    try:
        ...
        if table_id:
            # Delete from routing table (not implemented in builder yet)
            raise HTTPException(
                status_code=501,
                detail="Deleting routes from tables not yet implemented"
            )
        else:
            if route_type == "ipv4":
                builder.delete_ipv4_route(destination)
            elif route_type == "ipv6":
                builder.delete_ipv6_route(destination)
            else:
                raise HTTPException(
                    status_code=400,
                    detail="route_type must be 'ipv4' or 'ipv6'"
                )
        ...
        return VyOSResponse(
            success=response.status == 200,
            data={"message": f"Route {destination} deleted"},
            error=response.error if response.error else None
        )
...
async def set_route_map(route_map_name: str):
    ...
    try:
        ...
        builder.set_route_map(route_map_name)

        response = service.execute_batch(builder)

        return VyOSResponse(
            success=response.status == 200,
            data={"message": f"Route-map {route_map_name} set"},
            error=response.error if response.error else None
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def delete_route_map():
    ...
    try:
        ...
        builder.delete_route_map()

        response = service.execute_batch(builder)

        return VyOSResponse(
            success=response.status == 200,
            data={"message": "Route-map deleted"},
            error=response.error if response.error else None
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
