...
async def get_user_assignments(request: Request, user_id: str):
    ...
    async with db_pool.acquire() as conn:
        # Single query with LEFT JOIN to get assignments and permissions together
            ...
            SELECT
                ...
                COALESCE(
                    json_agg(
                        json_build_object(
                            'feature', ufp.feature,
                            'can_edit', ufp."canEdit",
                            'can_view', ufp."canView"
                        )
                    ) FILTER (WHERE ufp.id IS NOT NULL),
                    '[]'::json
                ) as feature_permissions
            FROM user_instance_roles uir
            JOIN instances i ON uir."instanceId" = i.id
            JOIN sites s ON i."siteId" = s.id
            LEFT JOIN user_feature_permissions ufp ON uir.id = ufp."userInstanceRoleId"
            WHERE uir."userId" = $1
            GROUP BY uir.id, uir."userId", uir."instanceId", i.name, i."siteId",
                     s.name, uir.role, uir."createdAt", uir."assignedBy"
        ...
        for assignment in assignments:
            # Parse JSON permissions from the aggregated column
            perms_data = assignment["feature_permissions"]
...
async def assign_user_to_instances(request: Request, body: AssignUserRequest):
    ...
    async with db_pool.acquire() as conn:
        ...
        # Use transaction to ensure atomicity - all assignments succeed or none
...
async def get_instance_users(request: Request, instance_id: str):
    ...
    async with db_pool.acquire() as conn:
        ...
        # Single query with LEFT JOIN to get users and permissions together
        # This eliminates N+1 queries
        users_data = await conn.fetch(
            ...
            GROUP BY u.id, u.name, u.email, uir.role
        ...
        for user in users_data:
            ...
            # Parse JSON permissions for OPERATOR/VIEWER roles
            if role in ["OPERATOR", "VIEWER"]:
                perms_data = user["feature_permissions"]
                if isinstance(perms_data, str):
                    perms_data = json.loads(perms_data)
