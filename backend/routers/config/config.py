...
async def get_config_snapshot(request: Request):
    ...
    try:
        ...
        snapshot = get_saved_config(instance_id)
        if snapshot is None:
            snapshot = await run_in_threadpool(service.get_full_config, refresh=True)
            set_saved_config(instance_id, snapshot)
...
async def get_config_diff(request: Request):
    ...
    try:
        ...
        # Absorb changes made outside VyManager into the baseline so the banner
        # only reports VyManager's own pending edits. Returns None when this
        # instance has no baseline yet.
...
async def discard_config(request: Request):
    ...
    try:
        ...
        # Reconcile first. Without this, discard diffs against a baseline that
        # has not absorbed external changes yet, so it also reverts edits made
        # by SSH or the CLI since the last poll (#855).
        baseline = reconcile_baseline(instance_id, current_config)
        if baseline is None:
...
async def confirm_commit(request: Request):
    ...
    try:
        ...
        # The confirmed commit is a VyManager change, so record it as managed.
        current_config = await run_in_threadpool(service.get_full_config, refresh=True)
