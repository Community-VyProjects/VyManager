...
class FirewallRulePacketMods(BaseModel):
    ...
    ttl: Optional[str] = None  # hop-limit for IPv6
...
class FirewallRuleTTLMatch(BaseModel):
    """Hop-limit matching configuration (IPv6 equivalent of TTL)."""
...
async def get_firewall_ipv6_config(http_request: Request, refresh: bool = False):
    ...
    try:
        ...
        def parse_rule(rule_num: str, rule_data: dict, chain: str, is_custom: bool = False) -> FirewallRule:
            ...
            # Parse ICMPv6
            icmp_type_name = None
            icmp_data = rule_data.get("icmpv6", {})
            if icmp_data:
            ...
            # Parse hop-limit match
            ttl_match = None
            ttl_data = rule_data.get("hop-limit", {})
            if ttl_data:
...
async def firewall_ipv6_batch_configure(http_request: Request, request: FirewallBatchRequest):
    ...
    try:
        ...
        for operation in request.operations:
            ...
            if operation.value and any(p in params for p in ["value", "description", "address", "port", "protocol", "action", "interface", "interface_name", "dscp", "mark", "hop_limit", "icmpv6_type", "target", "flag", "group_name", "mac_address", "country_code", "mac", "rate", "burst", "level", "count"]):
    ...
    try:
        ...
        for rule_item in request.rules:
            ...
            if rule_data.get("packet_mods"):
                ...
                if mods.get("hop_limit"):
                    builder.set_rule_set_hop_limit(request.chain, new_number, mods["hop_limit"], request.is_custom_chain)
            ...
            if rule_data.get("ttl_match"):
                ...
                if ttl.get("eq"):
                    builder.set_rule_hop_limit_eq(request.chain, new_number, ttl["eq"], request.is_custom_chain)
                if ttl.get("gt"):
                    builder.set_rule_hop_limit_gt(request.chain, new_number, ttl["gt"], request.is_custom_chain)
                if ttl.get("lt"):
                    builder.set_rule_hop_limit_lt(request.chain, new_number, ttl["lt"], request.is_custom_chain)
            ...
            if rule_data.get("icmp_code"):
                builder.set_rule_icmpv6_code(request.chain, new_number, rule_data["icmp_code"], request.is_custom_chain)
            if rule_data.get("icmpv6_type"):
                builder.set_rule_icmpv6_type(request.chain, new_number, rule_data["icmpv6_type"], request.is_custom_chain)
