...
export function DeleteDummyModal({
  open,
  onOpenChange,
  onSuccess,
  interfaceData,
}: DeleteDummyModalProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const result = await dummyService.deleteInterface(interfaceData.name);

      if (result.success) {
        onOpenChange(false);
        onSuccess();
      } else {
        setError(result.error || "Failed to delete dummy interface");
      }
    } catch (err) {
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription className="space-y-2">
            <p>
              Are you sure you want to delete this dummy interface? This action
              cannot be undone.
            </p>
