...
function PeerRow({ peer }: { peer: BgpPeerData }) {
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-2 text-sm border-b last:border-0">
      ...
      {peer.remote_as != null && (
        <span className="text-xs text-muted-foreground font-mono shrink-0" title={t("bgp.remoteAs")}>
          AS{peer.remote_as}
        </span>
      )}
      <div className="flex-1" />
      {peer.established && (
        <span className="flex items-center gap-1.5 text-[11px] text-muted-foreground shrink-0 tabular-nums" title={t("bgp.prefixes")}>
      ...
      {peer.uptime && (
        <span className="flex items-center gap-1 text-[11px] text-muted-foreground shrink-0" title={t("bgp.uptime")}>
  ...
  return (
    <div>
      <div className="flex items-center gap-2 px-3 py-1.5 bg-muted/40 border-b text-[11px] text-muted-foreground sticky top-0">
        ...
        {family.local_as != null && <span className="font-mono">{t("bgp.localAs", { as: String(family.local_as) })}</span>}
        {family.router_id && <span className="font-mono opacity-70">{t("bgp.routerId", { id: family.router_id })}</span>}
        <div className="flex-1" />
        <span className="tabular-nums">{t("bgp.peers", { count: family.peers.length })}</span>
      </div>
      {family.peers.map((p) => (
    ...
    <Card className="flex flex-col h-full">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3 shrink-0">
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-lg font-medium">{t("bgp.title")}</CardTitle>
          {status && status.total_peers > 0 && (
            <Badge variant="outline" className="text-[10px] h-5 px-1.5 font-medium">
              {t("upCount", { up: status.established_peers, total: status.total_peers })}
            </Badge>
          )}
        </div>
