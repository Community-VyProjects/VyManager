...
import { ChevronDown, Shield, Network, Server, Settings, LayoutDashboard, Route, Lock, LogOut, User, FileText, Building2, Power, PowerOff, Activity, LayoutGrid, Search, Command } from "lucide-react";
  ...
  },
  {
    ...
    children: [
      ...
      },
      {
        title: "VXLAN / EVPN",
        href: "/network/vxlan",
        requiredPermission: FeatureGroup.VXLAN
      },
      {
        title: "Interfaces",
    ...
    children: [
      {
        ...
        requiredPermission: FeatureGroup.SERVICES
      },
      {
    ...
    return items.map((item) => {
      ...
      if (item.children) {
        const visibleChildren = item.children.filter((child) => {
          ...
          // Special case for Services: show if user has SERVICES
          // OR any individual service permission
          if (child.requiredPermission === FeatureGroup.SERVICES) {
            return canRead(FeatureGroup.SERVICES) ||
                   canRead(FeatureGroup.DNS) ||
                   canRead(FeatureGroup.NTP) ||
                   canRead(FeatureGroup.SSH) ||
                   canRead(FeatureGroup.DHCP_RELAY) ||
                   canRead(FeatureGroup.SNMP) ||
                   canRead(FeatureGroup.LLDP);
          }
    ...
    <div className="flex h-screen w-64 flex-col border-r border-border bg-card">
      ...
      <div className="px-3 py-2">
        <button
          ...
          className="w-full flex items-center gap-2 px-3 py-2 text-sm text-muted-foreground bg-muted/50 hover:bg-muted rounded-md border transition-colors"
        >
