...
export function CreateRouteRuleModal({
  open,
  onOpenChange,
  onSuccess,
  policyType,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await routeService.createRule(policyType, policyName, ruleNumber, {
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Create Rule for Policy: {policyName}</DialogTitle>
          <DialogDescription>
            Add a new rule to the {policyType === "route" ? "IPv4" : "IPv6"} policy
          </DialogDescription>
        </DialogHeader>
