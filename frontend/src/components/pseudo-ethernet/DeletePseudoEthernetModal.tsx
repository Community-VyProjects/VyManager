  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(result.error || "Failed to delete pseudo-ethernet interface");
      }
    } catch (err) {
      setError((err as ApiError).message || "Failed to delete pseudo-ethernet interface");
    } finally {
      setLoading(false);
    }
  };
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription className="space-y-2">
            <p>
              Are you sure you want to delete this pseudo-ethernet interface? This action
              cannot be undone.
            </p>
            <div className="rounded-lg bg-muted/50 border p-3 mt-2">
              <p className="text-sm text-muted-foreground">
                ...
                {interfaceData.mode && (
                  <> &middot; Mode: <span className="font-medium text-foreground">{interfaceData.mode}</span></>
                )}
