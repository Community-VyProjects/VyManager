...
export default function BGPASPage() {
  ...
  if (loading) {
    return (
      <AppLayout>
        <LoadingState message="Loading AS path lists..." className="h-full" />
      </AppLayout>
    );
  }
  ...
  if (error) {
    return (
      <AppLayout>
        <ErrorState
          title="Error Loading AS Path Lists"
          message={error}
    ...
    <AppLayout>
      <div className="flex flex-col lg:flex-row h-full">
        ...
        <div className="lg:hidden border-b border-border p-3">
          <Select value={selectedAsPathList || ""} onValueChange={handleAsPathListSelect}>
            <SelectTrigger className="w-full">
              <SelectValue placeholder="Select AS path list..." />
            </SelectTrigger>
            <SelectContent>
              {asPathLists.map((apl) => (
                <SelectItem key={apl.name} value={apl.name}>{apl.name} ({apl.rules.length} rules)</SelectItem>
              ))}
            </SelectContent>
          </Select>
        </div>
        ...
        <div className="w-80 border-r border-border bg-card/50 hidden lg:flex flex-col">
          <div className="p-4 sm:p-6 pb-4 shrink-0">
            <PageHeader
              title="BGP AS Path"
              subtitle={`${asPathLists.length} ${asPathLists.length !== 1 ? "lists" : "list"} · ${totalRules} rule${totalRules !== 1 ? "s" : ""}`}
