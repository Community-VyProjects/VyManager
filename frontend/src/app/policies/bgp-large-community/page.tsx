...
export default function BGPLargeCommunityPage() {
  ...
  if (loading) {
    return (
      <AppLayout>
        <LoadingState message="Loading large community lists..." className="h-full" />
      </AppLayout>
    );
  }
  ...
  if (error) {
    return (
      <AppLayout>
        <ErrorState
          title="Error Loading Large Community Lists"
          message={error}
        ...
        <div className="w-80 border-r border-border bg-card/50 hidden lg:flex flex-col">
          <div className="p-4 sm:p-6 pb-4 shrink-0">
            <PageHeader
              ...
              subtitle={`${largeCommunityLists.length} ${largeCommunityLists.length !== 1 ? "lists" : "list"} · ${totalRules} rule${totalRules !== 1 ? "s" : ""}`}
