...
import { communityListService, CommunityList, CommunityListCapabilities } from "@/lib/api/community-list";
...
function CommunityListRuleRow({ rule, onEdit, onDelete }: any) {
...
export default function BGPCommunityPage() {
  ...
  if (loading) {
    return (
      <AppLayout>
        <LoadingState message="Loading community lists..." className="h-full" />
      </AppLayout>
    );
  }
  ...
  if (error) {
    return (
      <AppLayout>
        <ErrorState
          title="Error Loading Community Lists"
          message={error}
    ...
    <AppLayout>
      <div className="flex flex-col lg:flex-row h-full">
        ...
        <div className="lg:hidden border-b border-border p-3">
          <Select value={selectedCommunityList || ""} onValueChange={handleCommunityListSelect}>
        ...
        <div className="w-80 border-r border-border bg-card/50 hidden lg:flex flex-col">
          <div className="p-4 sm:p-6 pb-4 shrink-0">
            <PageHeader
              title="BGP Community"
              subtitle={`${communityLists.length} ${communityLists.length !== 1 ? "lists" : "list"} · ${totalRules} rule${totalRules !== 1 ? "s" : ""}`}
