...
export class FirewallFamilyService {
  constructor(private family: "ipv4" | "ipv6" = "ipv4") {}
  /**
   * Get capabilities based on VyOS version
   */
  async getCapabilities(): Promise<FirewallCapabilitiesResponse> {
    return apiClient.get<FirewallCapabilitiesResponse>(`/vyos/firewall/${this.family}/capabilities`);
  }
  ...
  async getConfig(refresh: boolean = false): Promise<FirewallConfigResponse> {
    return apiClient.get<FirewallConfigResponse>(`/vyos/firewall/${this.family}/config`, {
  ...
  async batchConfigure(request: FirewallBatchRequest): Promise<VyOSResponse> {
    const result = await apiClient.post<VyOSResponse>(`/vyos/firewall/${this.family}/batch`, request);
  ...
  async reorderRules(request: ReorderFirewallRequest): Promise<VyOSResponse> {
    const result = await apiClient.post<VyOSResponse>(`/vyos/firewall/${this.family}/reorder`, request);
    await this.refreshConfig();
    return result;
  }
  ...
  async createRule(
    ...
    if (config.ttl_match) {
      if (config.ttl_match.eq) {
        operations.push({ op: this.family === "ipv4" ? "set_rule_ttl_eq" : "set_rule_hop_limit_eq", value: config.ttl_match.eq });
      }
      if (config.ttl_match.gt) {
        operations.push({ op: this.family === "ipv4" ? "set_rule_ttl_gt" : "set_rule_hop_limit_gt", value: config.ttl_match.gt });
      }
      if (config.ttl_match.lt) {
        operations.push({ op: this.family === "ipv4" ? "set_rule_ttl_lt" : "set_rule_hop_limit_lt", value: config.ttl_match.lt });
      }
    }
  ...
  async updateRule(
    ...
    if (hasChanged(config.ttl_match, currentRule.ttl_match)) {
      if (currentRule.ttl_match) {
        operations.push({ op: this.family === "ipv4" ? "delete_rule_ttl" : "delete_rule_hop_limit" });
      }
      if (config.ttl_match) {
        if (config.ttl_match.eq) {
          operations.push({ op: this.family === "ipv4" ? "set_rule_ttl_eq" : "set_rule_hop_limit_eq", value: config.ttl_match.eq });
...
export const firewallIPv4Service = new FirewallFamilyService("ipv4");
export const firewallIPv6Service = new FirewallFamilyService("ipv6");
