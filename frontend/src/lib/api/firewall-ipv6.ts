...
class FirewallIPv6Service {
  ...
  async createRule(
    ...
    // Hop-limit matching (IPv6 equivalent of TTL)
    if (config.ttl_match) {
      if (config.ttl_match.eq) {
        operations.push({ op: "set_rule_hop_limit_eq", value: config.ttl_match.eq });
      }
      if (config.ttl_match.gt) {
        operations.push({ op: "set_rule_hop_limit_gt", value: config.ttl_match.gt });
      }
      if (config.ttl_match.lt) {
        operations.push({ op: "set_rule_hop_limit_lt", value: config.ttl_match.lt });
      }
    }
  ...
  async updateRule(
    ...
    // Update hop-limit matching (IPv6 equivalent of TTL)
    if (hasChanged(config.ttl_match, currentRule.ttl_match)) {
      if (currentRule.ttl_match) {
        operations.push({ op: "delete_rule_hop_limit" });
      }
      if (config.ttl_match) {
        if (config.ttl_match.eq) {
          operations.push({ op: "set_rule_hop_limit_eq", value: config.ttl_match.eq });
